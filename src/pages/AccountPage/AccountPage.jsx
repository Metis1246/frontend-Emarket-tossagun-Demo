...
function AccountPage() {
    ...
    if (user) {
      ...
      try {
        const res = await axios.get(
          `${apiUrl}/orderproduct/bycustomer/${user_id}`
        );
  ...
  const getLatestStatus = (statusDetails) => {
    if (!Array.isArray(statusDetails) || statusDetails.length === 0)
      return null;
  ...
  const statusCounts = userOrders
    ? userOrders.reduce((counts, order) => {
        ...
        const statusDetails = Object.values(statusEvents).find(
          (status) => status.value === latestStatusDetail.status
        );
        if (statusDetails) {
  ...
  const filteredOrders = (
    activeOrderStatus === "all"
      ...
      ? userOrders.filter((order) => {
          ...
          const orderStatus = Object.values(statusEvents).find(
            (status) => status.value === latestStatusDetail.status
          );
  ...
  const handleReceived = async (order_id, e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/orderproduct/receive/${order_id}`
      );
      ...
      console.error(
        "Error while receiving the order:",
  ...
  const StatusBar = () => (
    <ul
      className="flex gap-4 overflow-x-auto  md:gap-4 white-space-nowrap p-3 m-0 border-round-xl bg-white border border-gray-300 rounded-2xl border-1"
      style={{
        scrollbarWidth: "none",
    ...
    >
      {[
        { status: "all", label: "ทั้งหมด", count: userOrders?.length },
        {
          status: "รอชำระเงิน",
          label: "รอชำระเงิน",
          count: statusCounts[statusEvents?.Pending.key],
        },
        {
          status: "กำลังเตรียมจัดส่ง",
          label: "กำลังเตรียมจัดส่ง",
          count: statusCounts[statusEvents?.Packaged.key],
        },
        {
          status: "จัดส่งแล้ว",
          label: "จัดส่งแล้ว",
          count: statusCounts[statusEvents.Delivering.key],
        },
        {
          status: "รับสินค้าแล้ว",
          label: "รับสินค้าแล้ว",
          count: statusCounts[statusEvents.Received.key],
        },
        {
          status: "ยกเลิกออเดอร์",
          label: "ยกเลิกออเดอร์",
          count: statusCounts[statusEvents.Cancelled.key],
        },
      ].map((item) => (
        <li
          key={item.status}
          className={`list-none py-1 px-2 md:px-3 cursor-pointer border-round-lg text-xs md:text-sm  ${
        ...
        selectedOrderId ? (
          ...
          <div className="mb-3">
            ...
            <div className=" p-2 pl-0 border-round-top-xl">
              <h2 className="m-0 text-900 font-extrabold  text-4xl">
                การสั่งซื้อของฉัน
              </h2>
            </div>
  ...
  const OrderItem = ({ order }) => {
    const latestStatus =
      order.statusdetail
        .slice()
        .sort((a, b) => new Date(b.date) - new Date(a.date))[0]?.status ||
      "Unknown Status";
    const latestStatusEvent = Object.values(statusEvents).find(
      (event) => event.value === latestStatus
    );
    const tagCSS =
      latestStatusEvent?.tagCSS || "bg-gray-100 border-0 text-gray-700";
    return (
      <>
        <div className="hidden md:block w-full grid-nogutter bg-white border-1 surface-border border-round-xl p-3 mt-3 align-items-start">
          <div className="w-full border-bottom-1 surface-border pb-2 mb-2">
            <div className="flex justify-content-between">
              <Link
                to={`/ShopPage/${order.partner_id}`}
                className="no-underline text-900"
              >
                <div className="flex align-items-center">
                  <i className="pi pi-shop"></i>
                  <p className="m-0 ml-2 p-0 font-semibold">
                    ผู้ขาย: {order.partner_name}
                  </p>
                </div>
              </Link>
              <p className={`w-fit m-0 px-1 py-0 border-round-md ${tagCSS}`}>
                {latestStatus}
              </p>
            </div>
          </div>
          <div className="w-full">
            <div className="w-full flex flex-column text-left gap-2">
              {order.product.map((product, index) => (
                <div
                  key={index}
                  className="cart-items flex justify-content-between align-items-center border-bottom-1 surface-border pb-2"
                >
          ...
          <div className="w-full flex justify-content-end">
            <p className="mt-2 p-0 text-sm">
              <i className="pi pi-shopping-cart mr-1"></i>วันที่สั่งซื้อ:{" "}
          ...
          <div className="flex align-items-center justify-content-end pb-2 mt-2 md:mt-0">
            ...
            <p className="m-0 p-0 pr-2 font-semibold text-900">
              ฿{order.totalproduct?.toLocaleString("en-US")}
            </p>
          </div>
          <div className="flex align-items-center justify-content-end pb-2 mt-2 md:mt-0">
            <p className="m-0 p-0 mr-2">รวมค่าส่งทั้งหมด:</p>
            <p className="m-0 p-0 pr-2 font-semibold text-900">
              ฿{order.totaldeliveryPrice?.toLocaleString("en-US")}
            </p>
          </div>
          <div className="flex align-items-center justify-content-end pb-2 mt-2 md:mt-0">
            <p className="m-0 p-0 mr-2">รวมคำสั่งซื้อ:</p>
            <p className="m-0 p-0 pr-2 font-semibold text-900">
              ฿{order.alltotal?.toLocaleString("en-US")}
            </p>
          </div>
          <div className="w-full flex justify-content-end">
            {latestStatus === "จัดส่งแล้ว" ? (
              <Button
                label="ฉันได้รับสินค้าแล้ว"
                onClick={(e) => handleReceived(order?._id, e)}
              />
            ) : (
              ""
            )}
        ...
        <div className="block md:hidden w-full grid-nogutter bg-white border-1 surface-border border-round-xl p-3 mt-3 align-items-start">
          ...
          <div className="w-full">
            <div className="w-full py-2 flex flex-column text-left gap-2">
              {order.product.map((product, index) => (
                <div
                  key={index}
                  className="cart-items flex justify-content-between align-items-center pb-1"
                >
                  <div className="w-full flex">
                    ...
                    <div className="w-full flex flex-column justify-content-between ml-3">
                      <div className="flex flex-column">
                        <span className="max-w-17rem font-semibold text-sm  white-space-nowrap overflow-hidden text-overflow-ellipsis">
                          {product.product_name}
                        </span>
                        <span className="p-0 m-0 font-thin text-sm text-right text-400">
                          x{product.product_qty}
                        </span>
                      </div>
                      <span className="text-ml text-right font-semibold">
                        ฿
                        {Number(
                          product.product_price * product.product_qty
                        ).toLocaleString("en-US")}
          ...
          <div className="w-full flex justify-content-end align-items-center">
            <p className="m-0 p-0 text-right">
              สินค้ารวม {order?.product?.length} รายการ:{" "}
            </p>
            <p className="m-0 ml-1 p-0 text-right font-semibold">
              ฿{order.totalproduct?.toLocaleString("en-US")}
            </p>
          </div>
          <div className="w-full flex justify-content-end align-items-center">
            <p className="my-1 p-0 text-right text-900 text-l">
              รวมค่าส่งทั้งหมด:
            </p>
            <p className="m-0 ml-1 p-0 text-right font-semibold">
              ฿{order.totaldeliveryPrice?.toLocaleString("en-US")}
            ...
            {tabs.map((tab) => (
              <li
                ...
                className={`list-none py-3 cursor-pointer ${
                  activeTab === tab.id ? "text-yellow-500" : ""
                }`}
                onClick={() => {
                  setActiveTab(tab.id);
                  if (tab.id === "contactUs") {
